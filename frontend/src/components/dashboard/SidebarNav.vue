<template>
  ...
  <div v-if="showLogoutModal" class="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
    <div class="bg-white rounded-2xl p-6 w-full max-w-sm shadow-xl transition-all" @click.stop>
      ...
      <p class="text-gray-600 mb-6 text-sm">Apakah kamu yakin ingin keluar dari aplikasi?</p>
  ...
  >
    ...
    <div class="p-4 border-t border-gray-100 bg-surface-container-low transition-all cursor-default" :class="isCollapsed ? 'md:px-2' : ''" @click.stop>
      ...
      <div class="space-y-1">
        ...
        <button @click.stop="confirmLogout" class="w-full flex items-center px-4 py-2 text-sm text-error hover:bg-error/10 rounded-lg font-medium transition-colors mt-1" :class="isCollapsed ? 'md:justify-center' : 'gap-3'" :title="isCollapsed ? 'Keluar Aplikasi' : ''">
